...
# This is the receiver thread executing function to recv and yield data to output file
def recv_yield(file_name, address, port, local_socket):
    received = recv(file_name, address, port, local_socket)
    #once data is fully yeilded, output data into a file
    with open(file_name, 'wb+') as dl:
        for data in received:
            dl.write(data)

# Same function in basice Receiver
...
def usage():
    prompt = "Usage: python Receiver2.py Outputfile ReceiverAddress ReceiverPort Inputfile SenderAddress SenderPort"
    print(prompt)
    exit()
...
def main():
    ...
    # create a sender socket to send files for bonus part3
    Inputfile = None
    SenderAddress = None
    SenderPort = None
    # t1 thread to normally recv for Receiver, t2 thread to simultaneously send
    t1 = Thread(target=recv_yield, args=(file_name, address, port, local_socket))
    t2 = None
    ...
    if(len(sys.argv) > 4):
        ...
        sender_sock.bind(('', 9000))
        t2 = Thread(target=send, args=(Inputfile, SenderAddress, SenderPort, sender_sock))
    ...
    if t2: # if sys.argv > 4, it indicates it will both be sender and receiver
        ...
        t1.join()
        t2.join()
    else: # if sys.argv = 4 as usual, it indicates it will only serve as receiver
        t1.start()
        t1.join()
